...
int main(int argc, char** argv) {
  ...
  for (int index = 0; index < 10; index++) {
    ...
    if (index == 0) {
      ...
      for (int x = 10; x < gray.cols - 10; x++)
        for (int y = 10; y < gray.rows - 10; y++) {
          ...
          if (d == 0) continue;
          Eigen::Vector3d p3d =
              project2Dto3D(x, y, d, fx, fy, cx, cy, depth_scale);
    ...
    for (Measurement m : measurements) {
      ...
      Eigen::Vector2d pixel_prev =
          project3Dto2D(p(0, 0), p(1, 0), p(2, 0), fx, fy, cx, cy);
      Eigen::Vector3d p2 = Tcw * m.pos_world;
      Eigen::Vector2d pixel_now =
          project3Dto2D(p2(0, 0), p2(1, 0), p2(2, 0), fx, fy, cx, cy);
      ...
      img_show.ptr<uchar>(pixel_now(1, 0) +
                          color.rows)[int(pixel_now(0, 0)) * 3] = b;
      img_show.ptr<uchar>(pixel_now(1, 0) +
                          color.rows)[int(pixel_now(0, 0)) * 3 + 1] = g;
      img_show.ptr<uchar>(pixel_now(1, 0) +
                          color.rows)[int(pixel_now(0, 0)) * 3 + 2] = r;
      cv::circle(img_show, cv::Point2d(pixel_prev(0, 0), pixel_prev(1, 0)), 4,
                 cv::Scalar(b, g, r), 2);
      cv::circle(img_show,
                 cv::Point2d(pixel_now(0, 0), pixel_now(1, 0) + color.rows), 4,
                 cv::Scalar(b, g, r), 2);
    }
...
bool poseEstimationDirect(const vector<Measurement>& measurements,
                          ...
                          Eigen::Isometry3d& Tcw) {
  // 初始化g2o
  typedef g2o::BlockSolver<g2o::BlockSolverTraits<6, 1>>
  ...
  // g2o::OptimizationAlgorithmGaussNewton( solver_ptr ); // G-N
