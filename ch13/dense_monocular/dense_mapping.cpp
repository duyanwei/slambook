...
// 根据新的图像更新深度估计
bool update(const Mat& ref, const Mat& curr, const SE3& T_C_R, Mat& depth,
            Mat& depth_cov);

// 极线搜索
bool epipolarSearch(const Mat& ref, const Mat& curr, const SE3& T_C_R,
...
bool updateDepthFilter(const Vector2d& pt_ref, const Vector2d& pt_curr,
                       const SE3& T_C_R, Mat& depth, Mat& depth_cov);

// 计算 NCC 评分
double NCC(const Mat& ref, const Mat& curr, const Vector2d& pt_ref,
...
// 显示极线匹配
void showEpipolarMatch(const Mat& ref, const Mat& curr, const Vector2d& px_ref,
...
int main(int argc, char** argv) {
  ...
  for (int index = 1; index < color_image_files.size(); index++) {
    ...
    SE3 pose_T_C_R = pose_curr_TWC.inverse() *
...
bool readDatasetFiles(const string& path, vector<string>& color_image_files,
  ...
  while (!fin.eof()) {
    ...
    poses.push_back(SE3(Quaterniond(data[6], data[3], data[4], data[5]),
...
bool epipolarSearch(const Mat& ref, const Mat& curr, const SE3& T_C_R,
  ...
  Vector2d px_min_curr =
      cam2px(T_C_R * (f_ref * d_min));  // 按最小深度投影的像素
  Vector2d px_max_curr =
...
double NCC(const Mat& ref, const Mat& curr, const Vector2d& pt_ref,
  ...
  for (int x = -ncc_window_size; x <= ncc_window_size; x++)
    for (int y = -ncc_window_size; y <= ncc_window_size; y++) {
      ...
      double value_curr =
          getBilinearInterpolatedValue(curr, pt_curr + Vector2d(x, y));
...
void showEpipolarLine(const Mat& ref, const Mat& curr, const Vector2d& px_ref,
  ...
  cv::circle(curr_show, cv::Point2f(px_max_curr(0, 0), px_max_curr(1, 0)), 5,
             cv::Scalar(0, 255, 0), 2);
  cv::line(curr_show, Point2f(px_min_curr(0, 0), px_min_curr(1, 0)),
           Point2f(px_max_curr(0, 0), px_max_curr(1, 0)), Scalar(0, 255, 0), 1);
