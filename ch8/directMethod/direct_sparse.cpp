  ...
  for (int index = 0; index < 10; index++) {
    ...
    if (index == 0) {
      ...
      for (auto kp : keypoints) {
        ...
        if (d == 0) continue;
        Eigen::Vector3d p3d =
            project2Dto3D(kp.pt.x, kp.pt.y, d, fx, fy, cx, cy, depth_scale);
        float grayscale =
            float(gray.ptr<uchar>(cvRound(kp.pt.y))[cvRound(kp.pt.x)]);
    ...
    for (Measurement m : measurements) {
      ...
      cv::circle(img_show, cv::Point2d(pixel_prev(0, 0), pixel_prev(1, 0)), 8,
                 cv::Scalar(b, g, r), 2);
      cv::circle(img_show,
                 cv::Point2d(pixel_now(0, 0), pixel_now(1, 0) + color.rows), 8,
                 cv::Scalar(b, g, r), 2);
